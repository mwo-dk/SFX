namespace Gower.Tests

open Xunit
open SFX.FSharp.Gower
open SFX.TestHelpers
open Helpers

[<Trait("Category", "Unit")>]
module withAsymmetricDichotomousCharacterTests =
    let empty = {AsymmetricDichotomous = None; SymmetricDichotomous = None; Qualitative = None; Quantitative = None}
    let character (x: Sample) = true

    [<Fact>]
    let ``adding character to empty works``() =
        match (empty |> withAsymmetricDichotomousCharacter character).AsymmetricDichotomous with
        | Some l ->
            1 = (l |> List.length) |> isTrue
        | _ -> assertFail()

    [<Fact>]
    let ``adding character to non-empty works``() =
        let conf = empty |> withAsymmetricDichotomousCharacter character
        match (conf |> withAsymmetricDichotomousCharacter character).AsymmetricDichotomous with
        | Some l ->
            2 = (l |> List.length) |> isTrue
        | _ -> assertFail()

